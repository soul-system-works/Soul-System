...
**The bound (do not over-read this).** The form-not-substance evidence is against a *capable frontier baseline* on *moderate-to-tracer-hard* tasks, where carefulness rarely broke. A first probe of the regime that could overturn it — a *weaker* baseline that genuinely *fails* the same tasks (SOUL-I041, Haiku vs Opus, C1 compute-controlled) — sharpened rather than reversed the picture. Where the baseline breaks, a *single* role or gate still adds only form. The **integrated layer** (the whole seed + Mind) did supply real, compute-controlled substance — but in exactly **one** measured kind of place: **anchoring** (challenging an unverified claim it was about to build on), lifting a broken baseline. This replicated across two different anchor tasks (a vendor contract, an ad-budget reallocation), each time beating a length-matched neutral filler that left the baseline unmoved. A second discipline (Prophet / trajectory) looked like it generalised — until the compute control showed most of that gain was reproduced by length-matched *neutral filler*: flagging consequences is a disposition any long prompt elicits, whereas doubting a specific false premise needs the doctrine's actual content. So the layer's substance is **content-dependent and narrow**: it survives the compute control only on the discipline the Soul most repeats and most specifies. Everywhere else measured — generic capability (arithmetic, instruction-following) and the trajectory disposition — the gain is absent or a compute artifact, and the large context can even *distract*. And a capability gradient (Haiku → Sonnet → Opus) showed that even the anchoring substance is gated by whether the baseline *self-catches*, which rises with capability: an analysis-inviting task (recommend go/no-go) is self-caught by mid-capability and the substance is gone. A single high-stakes compliance task (G3b) once looked like the exception — there the whole layer *withheld* the artifact where a length-matched filler complied — but a direct curve-firming probe (three new compliance tasks × the full capability curve, n=5, C1-controlled, SOUL-I041 Step 8) **did not replicate that gap on any task at any capability point**. On those tasks the flag-the-flaw behaviour was reproduced by neutral filler (disposition-elicitable, the same way the Prophet/trajectory result dissolved under C1), and only the *form* — naming the Anchor Obligation / Skeptic, attempting external verification — was doctrine-attributable. The natural rescue — that G3b withheld because its artifact authorised a large *irreversible* commitment — was then **tested directly and refuted as doctrine** (SOUL-I041 Step 9): high-stakes irreversible twin tasks did raise withholding, but a length-matched neutral filler reproduced it (on a $4M non-cancellable contract the filler withheld 5/5, equal to the whole layer). So the apparent "compliance-resist edge toward the frontier" is **withdrawn**: stakes raise withholding, but disposition does, not the doctrine. What survives the compute control is narrower: legibility is the measured default; doctrine-attributable *substance* is real but **anchoring-specific and so far confined to weak baselines on content-dependent anchor tasks** (replicated n=2). Still bounded — expert-domain depth, far-longer horizon, higher n, and more disciplines/models remain under-tested; this is a measured shape, not a universal law. (Method rule it forced: run the equal-compute control on every apparent win — *three* "substance" results, trajectory, compliance-flag and compliance-withhold, were false positives without it.)
...
The Architect's obligations are precise. Honor the abstraction layer — what varies must be free to vary, what cannot vary must be enforced by the structure, what decides must live in the contracts between the parts. Record the structural plan before the Craftsman begins, so what was decided can be challenged and what was assumed can be seen. Structural decisions are recorded as a spec or ADR under `docs/specs/`; the Soul System's own doctrine decisions land in `amendments/`. Design for the work at hand, not for hypothetical futures.
